...
class unsupervised_kmeans():
        def kmeans_tut():      
            How_it_work,Try_algorithm=st.tabs(("How the algorithm work","Try Algorithm"))         
            with Try_algorithm:
                Try_col1,Try_col2=Try_algorithm.columns((5,5))
                n_clustersint=Try_col1.slider("No of clusters",2,15,1)
                #algorithm=Try_col1.selectbox("Algorithm",options=['lloyd', 'elkan', 'auto','full'])
                
               
                cluster_std=Try_col1.slider("Cluster standerd deviation",.5,10.0,.5)
                nsamples=Try_col2.slider("No of samples",100,1000,50)
                Go_model_BTN=Try_col1.button("Go")
                main_page.alignh(4,Try_col2)
                Generate_dataBTN =Try_col2.button("Generate New Data")
               
                if Go_model_BTN:

                    graph=unsupervised_kmeans.load_and_visualize(n_cluster=n_clustersint,cluster_std=cluster_std,nsamples=nsamples)
        ...
        #  with tabiris:
            ...
            with articlesData :   
                from Articles_classification import kmeans_text           
        ...
        def load_and_visualize(n_cluster,cluster_std,nsamples):
            
            #change number of centroids************************
            points, _ = make_blobs(cluster_std=cluster_std, n_samples=nsamples, n_features=2, random_state=1,)
            global centroids
            global closest
            ...
            centroids = initialize_centroids(points, n_cluster)
            closest = closest_centroid(points, centroids)
