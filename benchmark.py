...
class benchmark_classifiers(object):
    def __init__(self):
        ...
        selected_dataset=st.selectbox("Dataset",options=['Smart Irrigation','Iris','Smoke detection','Diabetes','Diseses','Image classifier','Article classifier','Digit recognition'])
        Benchmarker=st.button("Benchmark algorithms")
        if Benchmarker :
            if selected_dataset=='Smoke detection':
