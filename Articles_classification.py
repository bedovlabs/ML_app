...
class kmeans_text(object):
    ...
    def fit_model(self,x_train,y_train,n_culs,max_iteration,algorithm):

        model=KMeans(n_clusters=n_culs,algorithm=algorithm,max_iter=max_iteration)
        pipeline = Pipeline([('vect', TfidfVectorizer(ngram_range=(1, 2), stop_words="english", sublinear_tf=True)),
                            ('chi',  SelectKBest(chi2, k=10000)),
                            ('clf',KMeans(n_clusters=n_culs,algorithm=algorithm,max_iter=max_iteration))])

        #Fitting the model with the training data
        model = pipeline.fit(x_train, y_train)
        filename = 'savedmodels/kmeans_articles_model.sav'
        pickle.dump(model, open(filename, 'wb'))
        # showing  model accuracy score with validating and testing data
    def test_model(self,x_test,y_test):
        from sklearn.metrics import accuracy_score
        loaded_model = pickle.load(open('savedmodels/kmeans_articles_model.sav', 'rb'))
        y_pred=loaded_model.predict(x_test)
        return str(accuracy_score(y_pred,y_test)*100)
    def predict_input(self,user_input):
        loaded_model = pickle.load(open('savedmodels/kmeans_articles_model.sav', 'rb'))
